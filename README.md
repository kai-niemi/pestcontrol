...
Example of three individual hosts (`host1,host2,host3`):

```shell
scp target/pestcontrol-2.0.0-SNAPSHOT-bin.tar.gz user@host1:/~
scp target/pestcontrol-2.0.0-SNAPSHOT-bin.tar.gz user@host2:/~
scp target/pestcontrol-2.0.0-SNAPSHOT-bin.tar.gz user@host3:/~
ssh -t user@host1 'tar xvf pestcontrol-2.0.0-SNAPSHOT-bin.tar.gz && cd pestcontrol-2.0.0-SNAPSHOT && ./pop start-service'
ssh -t user@host2 'tar xvf pestcontrol-2.0.0-SNAPSHOT-bin.tar.gz && cd pestcontrol-2.0.0-SNAPSHOT && ./pop start-service'
ssh -t user@host3 'tar xvf pestcontrol-2.0.0-SNAPSHOT-bin.tar.gz && cd pestcontrol-2.0.0-SNAPSHOT && ./pop start-service'
```

Assuming you have 3 pest control instances running on 3 separate machines and a cluster configuration 
named `remote-insecure` with the IP/host names setup accordingly (see config section).
