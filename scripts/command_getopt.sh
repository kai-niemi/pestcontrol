...
fn_print_help() {
    ...
    echo -e "${cyan}Operator commands to start, stop and run pestcontrol${default}"
    {
        ...
        echo -e "${lightyellow}  stop-service|p\t${default}      Stop any running pestcontrol server"
        echo -e "${lightyellow}  run-service|r\t${default}      Run pestcontrol shell in interactive or non-interactive mode"
    } | column -s $'\t' -t

    echo -e ""
    echo -e "See 'pop run help' for an overview of the system."
}
